...
void ResonanceListView::paint(juce::Graphics& g)
{
    ...
    juce::DropShadow(theme::shadow.withAlpha(0.35f), 6, { 0, 2 }).drawForRectangle(g, bounds.toNearestInt());
    ...
    g.fillRoundedRectangle(bounds.reduced(3.0f).withHeight(bounds.getHeight() * 0.26f), 4.0f);
    g.setColour(theme::borderDark.withAlpha(0.65f));
    ...
    for (int i = 0; i < static_cast<int> (snapshot.peakCount); ++i)
    {
        ...
        g.setColour(theme::textOnDisplay.withAlpha(0.14f));
        ...
        g.setColour(theme::textOnDisplay.withAlpha(0.68f));
